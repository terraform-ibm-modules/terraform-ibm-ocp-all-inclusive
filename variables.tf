...
variable "operating_system" {
  ...
  default     = null
  validation {
    condition     = var.operating_system == null || var.operating_system == "RHEL 8" || var.operating_system == "RHCOS"
    error_message = "RHEL8 or Red Hat Enterprise Linux CoreOS (RHCOS) are the allowed OS values. RHCOS requires VPC clusters created from 4.15 onwards. Upgraded clusters from 4.14 cannot use RHCOS."
  }
}
